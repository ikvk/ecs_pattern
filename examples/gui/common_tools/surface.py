...
def pull_sf_onto_sf(sf: Surface, onto_sf: Surface, special_flags: int) -> Surface:
    """Натянуть sf на onto_sf - масштабирует sf, выводит результат на onto_sf и возвращает onto_sf"""
    scaled_sf = scale(sf.convert_alpha(), onto_sf.get_size())
    onto_sf.blit(scaled_sf, (0, 0), special_flags=special_flags)
    return onto_sf


def text_surface(font_obj: Font, value: Any, color_main: str, color_shadow: Optional[str] = None,
                 shadow_shift: float = 0.04) -> Surface:
...
def text_surface_ml(font_obj: Font, ml_text: str, color: str, width: float,
...
def colored_block_surface(color: Union[Color, int, str], width: int, height: int) -> Surface:
